...
export default function UnlockPage() {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (!res.ok) { const j = await res.json(); setError(j.error || "Gagal"); return; }
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3.5 bg-[var(--bg)] rounded-xl border border-[var(--border)]">
                  ...
                  <button onClick={() => { setFile(null); setError(""); setSuccess(false); }}
                    className="text-xs text-[var(--text-subtle)] hover:text-brand-500 font-medium transition-colors">Ganti</button>
                </div>

                <div>
                  <label className="label">Password PDF (jika ada)</label>
                  <div className="relative">
                    <input type={showPw ? "text" : "password"} value={password}
                      onChange={e => { setPassword(e.target.value); setError(""); }}
                      placeholder="Masukkan password jika ada…" className="input pr-10" />
                    <button onClick={() => setShowPw(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-subtle)]">
                      {showPw ? <EyeOff className="w-4 h-4"/> : <Eye className="w-4 h-4"/>}
                    </button>
                  </div>
                  <p className="text-[11px] text-[var(--text-subtle)] mt-1">Kosongkan jika PDF hanya memiliki restriksi cetak/edit</p>
                </div>
                ...
                {success && (
                  <div className="flex items-center gap-2 p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
                    ...
                    <p className="text-xs text-emerald-700 font-semibold">PDF berhasil di-unlock dan diunduh!</p>
                  </div>
                )}

                <Button onClick={handleUnlock} loading={processing} fullWidth size="lg" icon={<LockOpen className="w-5 h-5"/>}>
                  {processing ? "Memproses…" : "Unlock PDF"}
                </Button>
              </div>
            )}
          </div>
